...
class PublishRevisionImport extends AbstractIaOutboundJob
{
    public function perform(): void
    {
        ...
        $runId = $runs->createRun($this->job->getId(), $ownerId, 'metadata_revision_push', [
            'queue_ids' => $queueIds,
            'dry_run' => $dryRun,
        ]);
        ...
        foreach (array_chunk($queueIds, max(1, $chunkSize)) as $chunk) {
            $dispatcher->dispatch(PublishRevisionBatch::class, [
                'queue_ids' => $chunk,
                'run_id' => $runId,
                'dry_run' => $dryRun,
            ]);
